...
export default class LongList extends Thresh.Widget <any, any> {
  state = {}
  infos = [{
  ...
  handlePageScroll (e) {
    ...
    if (e.offset > 100 && !this.hasUpdate) {
      this.appBar.updateTitle('偏移大于 100')
      this.hasUpdate = !this.hasUpdate
    }
    if (e.offset < 100 && this.hasUpdate) {
      this.appBar.updateTitle('长列表渲染')
      this.hasUpdate = !this.hasUpdate
    }
  }
  ...
  render () {
    return (
      <Page
        appBar={
          <AppBar ref={ref => { this.appBar = ref }} title="长列表渲染" />
        }
      ...
      >
        <ScrollView onScroll={this.handlePageScroll.bind(this)} ref={ref => { this.scrollView = ref }}>
          <Button
            margin={20}
            padding={10}
            onTap={() => {
